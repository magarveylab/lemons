...
public class Chemoinformatics {
	...
	public static class Atoms {
		...
		/**
		 * Get the (last) atom connected to an atom in question.
		 * @param atom		atom in question
		 * @param molecule	molecule to search
		 * @return			connected atom
		 * @throws NoBondException 
		 */
		public static IAtom getConnectedAtom(IAtom atom, IAtomContainer molecule) 
			...
			if (bonds.size() == 0)
				throw new NoBondException("Error: no bonds to atom");
			IBond bond = bonds.get(bonds.size() - 1);
